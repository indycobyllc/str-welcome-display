(() => {
  ...
  function renderGuestHubQr(url) {
    ...
    link.href = url;
    link.hidden = true;
    if (!((_a = window.LocalQRCode) == null ? void 0 : _a.toDataURL)) {
      if (guestQrAttempt++ < 20) setTimeout(() => renderGuestHubQr(url), 250);
      return;
    }
    window.LocalQRCode.toDataURL(url, { width: 240, margin: 4, errorCorrectionLevel: "M" }).then((source) => {
      image.onload = () => {
        link.hidden = false;
        guestQrAttempt = 0;
      };
  ...
  function runParagraphChandelierShow() {
    ...
    setTimeout(() => display.classList.remove("chandelier-show"), 20 * 1e3);
  }
  setInterval(runParagraphChandelierShow, 5 * 60 * 1e3);
