...
function renderRequests() {
  ...
  pending.forEach(item => {
    ...
    if (!select || item.celebrationType !== "other") return;
    select.insertAdjacentHTML("beforeend", '<option value="other">Other celebration</option>'); select.value = "other";
    select.closest("label").insertAdjacentHTML("afterend", `<label class="wide">What they are celebrating<input data-request-field="celebrationOther" value="${escapeAdmin(item.celebrationOther)}"></label>`);
  });
}
