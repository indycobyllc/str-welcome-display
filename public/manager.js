...
$("managerForm").addEventListener("submit", async event => {
  ...
  const payload = { reservationName:$("reservationName").value.trim(), guestCount:Number($("guestCount").value), guestName:$("guestName").value.trim(), checkIn:$("checkIn").value, checkOut:$("checkOut").value };
